...
class StudioProject implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        $this->_model = [
            ...
            'fields' => [
                ...
                [
                    ...
                    'type' => 'StudioPage',
                    'nativeType' => NULL,
                    'relationName' => 'StudioPageToStudioProject',
                    'relationFromFields' => [],
                    'relationToFields' => [],
                    'isGenerated' => false,
                    'isUpdatedAt' => false,
        ...
        $this->_fieldsOnly = ['id', 'name', 'projectKey', 'projectType', 'data', 'html', 'css', 'createdAt', 'updatedAt'];
        $this->_tableFieldsOnly = ['id', 'name', 'projectKey', 'projectType', 'data', 'html', 'css', 'createdAt', 'updatedAt'];
        $this->_fieldsRelated = ['pages'];
        ...
        $this->_uniqueFields = ['projectKey'];
        $this->_primaryKeyAndUniqueFields = ['id', 'projectKey'];
        $this->_fieldsCombined = ['id', 'name', 'projectKey', 'projectType', 'data', 'html', 'css', 'createdAt', 'updatedAt', 'pages'];

        $this->_pdo = $pdo;
        $this->_dbType = $this->_pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        $this->_modelName = 'StudioProject';
        $this->_tableName = 'StudioProject';
    ...
    private function buildOneToManyRelationClause(string $relationName, array $field, string $nestedConditionSql, array $relatedFieldKeys, string $conditionType): string
    {
        ...
        $fromField = PPHPUtility::quoteColumnName($this->_dbType, $relatedFieldKeys['relationFromFields'][0]);
        $toField = PPHPUtility::quoteColumnName($this->_dbType, $relatedFieldKeys['relationToFields'][0]);

        switch ($conditionType) {
            case 'some':
                return "EXISTS(SELECT 1 FROM $relatedTable AS j0 " .
                    "WHERE $nestedConditionSql " .
                    "AND j0.$fromField = $quotedTableName.$toField)";
    ...
    /**
     * Performs an aggregate operation on the 'StudioProject' table. This method allows for flexible aggregation queries
     ...
     *
     * @return StudioProjectData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->studioProject->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'StudioProject' table.
    ...
    /**
     * Creates a new StudioProject in the database.
     *
     * This method is designed to insert a new StudioProject record into the database using provided data.
     * It is capable of handling related records through the dynamically defined relations in the StudioProject model.
     ...
     *
     * @return StudioProjectData The newly created StudioProjectData record.
     *
     * @throws:
     ...
     * Example:
     * ```
     * Example of creating a new StudioProject with related profile and roles
     * $result = $prisma->studioProject->create([
     *   'data' => [
     *     'property' => 'value',
     * ]);
     * ```
    ...
    /**
     * Bulk Create StudioProject – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple StudioProject records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many StudioProject records at once.
     *
     * Key Features:
     * - Accepts an array of StudioProject records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every StudioProject before saving.
     ...
     *   ];
     *   $result = $prisma->studioProject->createMany([
     *     'data' => $data,
     ...
     *
     * @param:
     * - array $data: Must include a 'data' key with an array of StudioProject records.
     * - skipDuplicates (optional): Set to true to ignore duplicates if supported by your DB.
     *
     * @return:
     * - An object like (object)['count' => N], where N is the number of StudioProjects created.
     *
     * @throws:
    ...
    /**
     * Bulk Create StudioProject and Return Created Records
     *
     * This method allows you to create multiple StudioProject records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the StudioProject records to create.
     ...
     *
     * @return array An array of created StudioProjectData objects.
     *
    ...
    /**
     * Retrieves a single StudioProject record matching specified criteria.
     * 
     * Searches for a unique StudioProject based on the provided filter criteria within `$criteria`.
     * It returns the StudioProject data as either an associative array or an object, based on the `$format` parameter.
     * This method supports filtering (`where`), field selection (`select`), and inclusion of related models (`include`).
     * If no matching StudioProject is found, an empty array is returned. The method includes comprehensive error handling for invalid inputs and parameter conflicts.
     *
     * @param array $criteria Filter criteria with keys:
     *  - 'where': Conditions to filter StudioProject records.
     *  - 'select': Fields of the StudioProject to return.
     *  - 'include': Related models to include in the result.
     * @return StudioProjectData|null The StudioProjectData record matching the criteria.
     * 
     ...
     * 
     * @example
     * To find a StudioProject by ID, select specific fields, and include related models:
     * $result = $prisma->studioProject->findUnique([
     *   'where' => ['id' => 'someUserId'],
    ...
    public function findUnique(array $criteria): ?object
    {
        ...
        if (empty($conditions)) {
            throw new Exception("No valid 'where' conditions provided for finding a unique record in StudioProject.");
        }
    ...
    /**
     * Retrieves a unique StudioProject record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one StudioProject to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique StudioProject, same as in `findUnique`.
     * @return StudioProjectData The StudioProjectData record matching the criteria.
     * @throws Exception If no StudioProject record matches the criteria.
     */
    ...
    /**
     * Retrieves multiple StudioProject records based on specified filter criteria.
     *
     ...
     * if no StudioProjects match the criteria. This approach ensures flexibility and efficiency in fetching data
     ...
     * @return StudioProjectData[] An array of StudioProjectData objects or an empty array
     * 
     * @example
     * Retrieve StudioProject with cursor-based pagination:
     * $result = $prisma->studioProject->findMany([
     *   'cursor' => ['id' => 'someUserId'],
     *   'take' => 5
     * ]);
     * 
     * Select specific fields of StudioProject:
     * $result = $prisma->studioProject->findMany([
    ...
    /**
     * Retrieves the first StudioProject record that matches specified filter criteria.
     *
     * Designed to efficiently find and return the first StudioProject record matching the provided criteria.
     ...
     *   - 'where': Filter criteria for searching StudioProject records.
     ...
     *
     * Examples:
     * Find a StudioProject by email, returning specific fields:
     * $result = $prisma->studioProject->findFirst([
     *   'where' => ['email' => 'user@example.com'],
     *   'select' => ['id', 'email', 'name']
     * ]);
     * Find an active StudioProject, include their posts, ordered by name:
     * $result = $prisma->studioProject->findFirst([
     *   'where' => ['active' => true],
     *   'orderBy' => 'name',
     *   'include' => ['posts' => true]
    ...
    /**
     * Retrieves the first StudioProject record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a StudioProject record is returned. If no record
     ...
     * 
     * Returns:
     * @return StudioProjectData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a StudioProject's email and only return their 'id' and 'email' in the response
     * $result = $prisma->studioProject->update([
     *   'where' => ['id' => 'someUserId'],
     *   'data' => ['email' => 'new.email@example.com'],
     *   'select' => ['id' => true, 'email' => true]
     * ]);
     * 
     * Example 2: Update a StudioProject's username and include their profile information in the response
     * $result = $prisma->studioProject->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    {
        if (!isset($data['where'])) {
            throw new Exception("The 'where' key is required in the update StudioProject.");
        }
        ...
        if (!isset($data['data'])) {
            throw new Exception("The 'data' key is required in the update StudioProject.");
        }
    ...
    /**
     * Bulk update StudioProject records with flexible conditions and smart validation.
     *
     * This method lets you update multiple StudioProject records at once, based on any conditions you specify.
     ...
     *
     * Example:
     *   $result = $prisma->studioProject->updateMany([
     *     'where' => ['name' => null],
     *     'data'  => ['name' => 'Anonymous']
     *   ]);
     *   $result = (object)['count' => 3]
     *
     * This is perfect for mass updates—like activating StudioProject, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple StudioProject records and returns the updated records.
     *
     * This method allows you to update multiple StudioProject records based on specified conditions
     ...
     *
     * @return array An array of updated StudioProjectData objects.
     *
     ...
     * @return StudioProjectData|array On successful deletion, returns the deleted StudioProjectData record.
     *               If the deletion is unsuccessful due to a non-existent StudioProjectData or non-unique criteria, returns an
     *               array with 'modelName' and 'cause' keys, indicating the reason for failure.
     * 
     * @example
     * Delete a StudioProject by ID and return the deleted User's data as an array
     * $result = $prisma->studioProject->delete([
     *   'where' => ['id' => 'someUserId']
     * ]);
     * 
     * @example
     * Delete a StudioProject by ID, selecting specific fields to return
     * $result = $prisma->studioProject->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'select' => ['name' => true, 'email' => true]
     * ]);
     * 
     * @example
     * Delete a StudioProject by ID, including related records in the return value
     * $result = $prisma->studioProject->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'include' => ['posts' => true]
     * ]);
    ...
    public function delete(array $criteria): object|array
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'StudioProject', 'cause' => 'Record to delete does not exist.'];
        } catch (Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    /**
     * Performs an Upsert Operation on a StudioProject Record
     *
     * Implements an "upsert" operation for StudioProject records. This method checks for the existence of a StudioProject based on
     * specified 'where' conditions. If the StudioProject exists, it updates the StudioProject with the provided 'update' data;
     * if the StudioProject does not exist, it creates a new StudioProject with the 'create' data. This operation is atomic, ensuring
     ...
     * 
     * @return StudioProjectData Returns the created or updated User record.
     * 
     * Exceptions:
     ...
     * 
     * Example Usage:
     * $result = $prisma->studioProject->upsert([
     *   'where' => ['email' => 'user@example.com'],
    ...
    /**
     * Groups records in the 'StudioProject' table based on specified criteria.
     * 
     * This function groups records in the 'StudioProject' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple StudioProject records based on flexible criteria.
     *
     * This method enables bulk deletion of StudioProject records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid StudioProject field(s) as filter criteria.
     ...
     *
     * Example:
     *   $result = $prisma->studioProject->deleteMany([
     ...
     * @return int|array|StudioProjectData The number of records that match the criteria or detailed count per field.
