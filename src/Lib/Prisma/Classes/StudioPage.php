...
class StudioPage implements IModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->_fieldsOnly = ['id', 'projectId', 'pageId', 'name', 'slug', 'html', 'css', 'isHome', 'sort', 'createdAt', 'updatedAt'];
        $this->_tableFieldsOnly = ['id', 'projectId', 'pageId', 'name', 'slug', 'html', 'css', 'isHome', 'sort', 'createdAt', 'updatedAt'];
        $this->_fieldsRelated = ['project'];
        ...
        $this->_fieldsCombined = ['id', 'projectId', 'pageId', 'name', 'slug', 'html', 'css', 'isHome', 'sort', 'createdAt', 'updatedAt', 'project'];

        $this->_pdo = $pdo;
        $this->_dbType = $this->_pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        $this->_modelName = 'StudioPage';
        $this->_tableName = 'StudioPage';
    ...
    /**
     * Performs an aggregate operation on the 'StudioPage' table. This method allows for flexible aggregation queries
     ...
     *
     * @return StudioPageData Returns an object containing the results of the aggregate operation. The result will include keys corresponding to the specified
     *
     ...
     *     'where' => ['status' => 'active'],
     * ];
     * $result = $prisma->studioPage->aggregate($criteria);
     ...
     * queries with conditions (WHERE clause), ordering (ORDER BY), and pagination (LIMIT and OFFSET) on the 'StudioPage' table.
    ...
    /**
     * Creates a new StudioPage in the database.
     *
     * This method is designed to insert a new StudioPage record into the database using provided data.
     * It is capable of handling related records through the dynamically defined relations in the StudioPage model.
     ...
     *
     * @return StudioPageData The newly created StudioPageData record.
     *
     * @throws:
     ...
     * Example:
     * ```
     * Example of creating a new StudioPage with related profile and roles
     * $result = $prisma->studioPage->create([
     *   'data' => [
     *     'property' => 'value',
     * ]);
     * ```
    ...
    /**
     * Bulk Create StudioPage – Fast, Safe, and Flexible
     *
     * Effortlessly add multiple StudioPage records to your database in a single, atomic operation.
     * This method is perfect for onboarding, migrations, or any scenario where you need to insert many StudioPage records at once.
     *
     * Key Features:
     * - Accepts an array of StudioPage records—add as many as you need in one call.
     * - Runs inside a transaction: if any record fails, nothing is saved.
     * - Validates required fields for every StudioPage before saving.
     ...
     *   ];
     *   $result = $prisma->studioPage->createMany([
     *     'data' => $data,
     ...
     *
     * @param:
     * - array $data: Must include a 'data' key with an array of StudioPage records.
     * - skipDuplicates (optional): Set to true to ignore duplicates if supported by your DB.
     *
     * @return:
     * - An object like (object)['count' => N], where N is the number of StudioPages created.
     *
     * @throws:
    ...
    /**
     * Bulk Create StudioPage and Return Created Records
     *
     * This method allows you to create multiple StudioPage records in a single operation and returns the created records.
     * It supports skipping duplicates and allows for selective field retrieval.
     *
     * @param array $data An associative array containing:
     * - 'data': A list of associative arrays representing the StudioPage records to create.
     ...
     *
     * @return array An array of created StudioPageData objects.
     *
    ...
    /**
     * Retrieves a single StudioPage record matching specified criteria.
     * 
     * Searches for a unique StudioPage based on the provided filter criteria within `$criteria`.
     * It returns the StudioPage data as either an associative array or an object, based on the `$format` parameter.
     * This method supports filtering (`where`), field selection (`select`), and inclusion of related models (`include`).
     * If no matching StudioPage is found, an empty array is returned. The method includes comprehensive error handling for invalid inputs and parameter conflicts.
     *
     * @param array $criteria Filter criteria with keys:
     *  - 'where': Conditions to filter StudioPage records.
     *  - 'select': Fields of the StudioPage to return.
     *  - 'include': Related models to include in the result.
     * @return StudioPageData|null The StudioPageData record matching the criteria.
     * 
     ...
     * 
     * @example
     * To find a StudioPage by ID, select specific fields, and include related models:
     * $result = $prisma->studioPage->findUnique([
     *   'where' => ['id' => 'someUserId'],
    ...
    public function findUnique(array $criteria): ?object
    {
        ...
        if (empty($conditions)) {
            throw new Exception("No valid 'where' conditions provided for finding a unique record in StudioPage.");
        }
    ...
    /**
     * Retrieves a unique StudioPage record based on specified criteria.
     *
     * This method is a wrapper around `findUnique` that throws an exception if no record is found.
     * It is useful when you expect exactly one StudioPage to match the criteria and want to handle the case where none exists.
     *
     * @param array $criteria Criteria to find the unique StudioPage, same as in `findUnique`.
     * @return StudioPageData The StudioPageData record matching the criteria.
     * @throws Exception If no StudioPage record matches the criteria.
     */
    ...
    /**
     * Retrieves multiple StudioPage records based on specified filter criteria.
     *
     ...
     * if no StudioPages match the criteria. This approach ensures flexibility and efficiency in fetching data
     ...
     * @return StudioPageData[] An array of StudioPageData objects or an empty array
     * 
     * @example
     * Retrieve StudioPage with cursor-based pagination:
     * $result = $prisma->studioPage->findMany([
     *   'cursor' => ['id' => 'someUserId'],
     *   'take' => 5
     * ]);
     * 
     * Select specific fields of StudioPage:
     * $result = $prisma->studioPage->findMany([
    ...
    /**
     * Retrieves the first StudioPage record that matches specified filter criteria.
     *
     * Designed to efficiently find and return the first StudioPage record matching the provided criteria.
     ...
     *   - 'where': Filter criteria for searching StudioPage records.
     ...
     *
     * Examples:
     * Find a StudioPage by email, returning specific fields:
     * $result = $prisma->studioPage->findFirst([
     *   'where' => ['email' => 'user@example.com'],
     *   'select' => ['id', 'email', 'name']
     * ]);
     * Find an active StudioPage, include their posts, ordered by name:
     * $result = $prisma->studioPage->findFirst([
     *   'where' => ['active' => true],
     *   'orderBy' => 'name',
     *   'include' => ['posts' => true]
    ...
    /**
     * Retrieves the first StudioPage record that matches specified filter criteria or throws an exception if not found.
     *
     * This method is similar to `findFirst`, but it guarantees that a StudioPage record is returned. If no record
     ...
     * 
     * Returns:
     * @return StudioPageData|array|null The updated User record or an empty array if no match is found.
     * 
     * Example Usage:
     * Example 1: Update a StudioPage's email and only return their 'id' and 'email' in the response
     * $result = $prisma->studioPage->update([
     *   'where' => ['id' => 'someUserId'],
     *   'data' => ['email' => 'new.email@example.com'],
     *   'select' => ['id' => true, 'email' => true]
     * ]);
     * 
     * Example 2: Update a StudioPage's username and include their profile information in the response
     * $result = $prisma->studioPage->update([
     *   'where' => ['id' => 'someUserId'],
    ...
    {
        if (!isset($data['where'])) {
            throw new Exception("The 'where' key is required in the update StudioPage.");
        }
        ...
        if (!isset($data['data'])) {
            throw new Exception("The 'data' key is required in the update StudioPage.");
        }
    ...
    /**
     * Bulk update StudioPage records with flexible conditions and smart validation.
     *
     * This method lets you update multiple StudioPage records at once, based on any conditions you specify.
     ...
     *
     * Example:
     *   $result = $prisma->studioPage->updateMany([
     *     'where' => ['name' => null],
     *     'data'  => ['name' => 'Anonymous']
     *   ]);
     *   $result = (object)['count' => 3]
     *
     * This is perfect for mass updates—like activating StudioPage, anonymizing data, or fixing typos—quickly and safely.
     */
    ...
    /**
     * Updates multiple StudioPage records and returns the updated records.
     *
     * This method allows you to update multiple StudioPage records based on specified conditions
     ...
     *
     * @return array An array of updated StudioPageData objects.
     *
     ...
     * @return StudioPageData|array On successful deletion, returns the deleted StudioPageData record.
     *               If the deletion is unsuccessful due to a non-existent StudioPageData or non-unique criteria, returns an
     *               array with 'modelName' and 'cause' keys, indicating the reason for failure.
     * 
     * @example
     * Delete a StudioPage by ID and return the deleted User's data as an array
     * $result = $prisma->studioPage->delete([
     *   'where' => ['id' => 'someUserId']
     * ]);
     * 
     * @example
     * Delete a StudioPage by ID, selecting specific fields to return
     * $result = $prisma->studioPage->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'select' => ['name' => true, 'email' => true]
     * ]);
     * 
     * @example
     * Delete a StudioPage by ID, including related records in the return value
     * $result = $prisma->studioPage->delete([
     *   'where' => ['id' => 'someUserId'],
     *   'include' => ['posts' => true]
     * ]);
    ...
    public function delete(array $criteria): object|array
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'StudioPage', 'cause' => 'Record to delete does not exist.'];
        } catch (Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    /**
     * Performs an Upsert Operation on a StudioPage Record
     *
     * Implements an "upsert" operation for StudioPage records. This method checks for the existence of a StudioPage based on
     * specified 'where' conditions. If the StudioPage exists, it updates the StudioPage with the provided 'update' data;
     * if the StudioPage does not exist, it creates a new StudioPage with the 'create' data. This operation is atomic, ensuring
     ...
     * 
     * @return StudioPageData Returns the created or updated User record.
     * 
     * Exceptions:
     ...
     * 
     * Example Usage:
     * $result = $prisma->studioPage->upsert([
     *   'where' => ['email' => 'user@example.com'],
    ...
    /**
     * Groups records in the 'StudioPage' table based on specified criteria.
     * 
     * This function groups records in the 'StudioPage' table according to the fields specified 
    ...
    /**
     * Dynamically deletes multiple StudioPage records based on flexible criteria.
     *
     * This method enables bulk deletion of StudioPage records by specifying any combination of conditions in the 'where' key.
     * - Accepts any valid StudioPage field(s) as filter criteria.
     ...
     *
     * Example:
     *   $result = $prisma->studioPage->deleteMany([
     ...
     * @return int|array|StudioPageData The number of records that match the criteria or detailed count per field.
